from promocode.models import Coupon, CouponUser
...
def validate_max_uses_rule(coupon_object):
...
def validate_max_uses_rule_for_user(coupon_object, user):
    max_uses_rule = coupon_object.ruleset.max_uses
    # if coupon_object.times_used >= max_uses_rule.max_uses and not max_uses_rule.is_infinite:
    #     return False
    try:
...
def validate_coupon(coupon_code, user):
    ...
    if not valid_validity_rule:
        return assemble_invalid_message(message="Invalid coupon! Coupon code expired")
    ...
    valid_max_uses_rule = validate_max_uses_rule(coupon_object=coupon_object)
    if not valid_max_uses_rule:
        return assemble_invalid_message(message="Coupon uses exceeded!")

    valid_max_uses_rule = validate_max_uses_rule_for_user(coupon_object=coupon_object, user=user)
    if not valid_max_uses_rule:
